...
class Window(QMainWindow):
	...
	def UiComponents(self):
		...
		# creating a push button
		push_root = QPushButton("sq.rt", self)

		# setting geometry
		push_root.setGeometry(5, 350, 80, 40)
		...
		#check last answer with equation
		push_checkpr = QPushButton("CheckPr", self)
		push_checkpr.setGeometry(5, 400, 80, 40)

		# QLabel for checking previous equations with answers
		self.labeleq = QLabel(self)

		# setting geometry to the label
		self.labeleq.setGeometry(95, 350, 250, 80)

		# creating label multi line
		self.labeleq.setWordWrap(True)

		# setting style sheet to the label
		self.labeleq.setStyleSheet("QLabel"
								   "{"
		...
		# setting alignment to the label
		self.labeleq.setAlignment(Qt.AlignRight)

		# setting font
		self.labeleq.setFont(QFont('Arial', 12))
		...
		push_checkpr.clicked.connect(self.action_checkpr)
	...
	def action_equal(self):
		...
		try:
			...
			#ans = eval(equation)
			d = Solution()
			ans = d.calculate(equation)

			# setting text to the main label and labeleq
			self.labeleq.setText(str(equation) + "=" + str(ans))
			self.label.setText(str(ans))
			self.checkeq.append(equation)
			self.checkans.append(ans)
			self.count += 1
			self.resetcount += 1
	...
	def notinsert(self):
		...
		if (text == ""):
			return "f"
		if len(text) > 1 and text[-1] in self.oplist:
				self.labeleq.setText("Cannot insert here")
				return "f"
		return "t"
	...
	def action_checkpr(self):
		#checking the last results
		if(self.count < 0):
			self.count = self.resetcount
		countfin = self.count
		self.labeleq.setText(str(self.checkeq[countfin]) + "=" + str(self.checkans[countfin]) + f"\n{self.count + 1}")
		self.count -= 1
